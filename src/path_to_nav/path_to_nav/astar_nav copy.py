...
3. Run A* on /map (static occupancy grid) with lightweight obstacle inflation
...
Why /map instead of /global_costmap/costmap
--------------------------------------------
The global costmap is only published when Nav2's global planner is active.
In an m-explore-ros2 post-exploration workflow the global costmap may not be
available. /map is always published by map_server and covers the full
environment, so it is the reliable choice for a custom global planner.
A small morphological dilation (INFLATION_RADIUS) is applied to raw /map
cells to compensate for the robot footprint — the same role the costmap
inflation layer would have played.
...
# ─────────────────────────────────────────────────────────────────────────────
OBSTACLE_THRESHOLD   = 65     # /map occupancy value (0-100) above which = obstacle
INFLATION_RADIUS     = 0.18   # m — robot half-width; cells within this distance
                               # of any obstacle are marked occupied before A*
...
VIZ_BATCH_SIZE       = 60     # explored nodes per RViz publish
VIZ_BATCH_DELAY      = 0.008  # s between batches (controls animation speed)
...
class AStarNav(Node):
    ...
    def __init__(self):
        ...
        # ── Subscribers ───────────────────────────────────────────────────────
        # Use /map (static occupancy grid from map_server) — always available
        # regardless of whether the global costmap is active.
        # We apply our own lightweight inflation below before running A*.
        self.create_subscription(
            OccupancyGrid, '/map', self._costmap_cb, 10
        )
        ...
        self.explored_pub = self.create_publisher(MarkerArray,   '/astar/explored', 10)
    ...
    def _plan_and_navigate(self, point_msg: PointStamped):
        ...
        # Build binary grid from /map and inflate obstacles by robot footprint
        raw  = np.array(costmap.data, dtype=np.int8).reshape(
            (info.height, info.width)
        )
        # /map: -1=unknown, 0=free, 1-100=occupied probability
        # Treat unknown cells (-1) as free so A* can cross unexplored edges
        binary = (raw >= OBSTACLE_THRESHOLD).astype(np.uint8)
        grid   = self._inflate(binary, info.resolution)
    ...
    def _publish_explored(self, explored: list, info):
        """
        Publish explored nodes as a batched MarkerArray so RViz shows the
        search frontier spreading outward (blue → green gradient = order of expansion).
        """
        # Clear old markers
        clear = MarkerArray()
        dm = Marker()
        ...
        clear.markers.append(dm)
        self.explored_pub.publish(clear)

        self.get_logger().info(
            f'Streaming {len(explored)} explored nodes to RViz...'
        )
        ...
        for idx, (r, c) in enumerate(explored):
            ...
            mk.scale.y = res * 0.85
            mk.scale.z = 0.01
            # Colour: blue (early) → cyan → green (late)
            mk.color.r = 0.0
            mk.color.g = t
            mk.color.b = 1.0 - 0.6 * t
            mk.color.a = 0.55
            mk.lifetime.sec = 60
            batch.markers.append(mk)
            mid += 1

            if len(batch.markers) >= VIZ_BATCH_SIZE:
                self.explored_pub.publish(batch)
                batch = MarkerArray()
                time.sleep(VIZ_BATCH_DELAY)

        if batch.markers:
            self.explored_pub.publish(batch)

        self.get_logger().info('✓ Explored node animation complete.')

    # =========================================================================
    # HELPERS
    # =========================================================================

    def _inflate(self, binary: 'np.ndarray', resolution: float) -> 'np.ndarray':
        """
        Morphological dilation — expand every obstacle cell outward by
        INFLATION_RADIUS metres so A* keeps the robot body clear of walls.
        This replicates what Nav2's inflation layer does on the costmap.
        """
        from scipy.ndimage import binary_dilation, generate_binary_structure
        radius_cells = max(1, int(math.ceil(INFLATION_RADIUS / resolution)))
        struct   = generate_binary_structure(2, 2)   # 8-connected kernel
        inflated = binary_dilation(binary, structure=struct, iterations=radius_cells)
        return inflated.astype('uint8')
