...
class AStarNav(Node):
    ...
    def __init__(self):
        ...
        # TF for map to baselink transforms
        self.tf_buffer   = tf2_ros.Buffer()
        self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)

    # Subscribers 
        # Global costmap already has inflation regions placed around 'obstables' from the map
        self.create_subscription(
            OccupancyGrid, '/global_costmap/costmap', self._costmap_cb, 10
        )

        # Inspected AMCL pose allows us to 
        self.create_subscription(
    ...
    # displayed be displayed in rviz2
    ...
    def _goal_cb(self, msg: PointStamped):
        ...
        # Run mission execution in a thread
        threading.Thread(
            target=self._execute_mission, args=(msg,), daemon=True
        ).start()

    # MISSION to relocalize then plan and then send goal 
