...
5. Publish final path to /astar/path  (Path)
...
OBSTACLE_THRESHOLD     = 253    # costmap value (0-254) where we assign hard obstacles
...
# MISSION 
    ...
    # Mission step 2 - plan the path using A*
    ...
    def _plan_and_navigate(self, point_msg: PointStamped):
        ...
        obstacle_mask = (raw >= OBSTACLE_THRESHOLD)
    ...
    def _astar(
        ...
        def h(r, c):
            return math.hypot(r - gr, c - gc)   # distance from current position to goal 
        ...
        while open_heap:
            ...
            # ignore if points have already been handled. Heap guarantees we've handled the minimum
            if (r, c) in closed:
                continue
            closed.add((r, c))
            explored.append((r, c))

            # if we've reached the goal, form the path 
            if (r, c) == (gr, gc):
                path, node = [], (gr, gc)
            ...
            for dr, dc, geo_cost in MOVES:
                ...
                if not (0 <= nr < H and 0 <= nc < W):
                    continue
                # and if part of obstable or already explored then ignore 
                ...
                # if the new cost is better than the current cost to that node, then update the 
                # minimum score and the movement that caused a minimal score 
                tg = g + move_cost
