...
Scenario selection (pass as ROS parameter at startup):
    ros2 run mission_manager slam_and_explore --ros-args -p scenario:=sandbox
    ros2 run mission_manager slam_and_explore --ros-args -p scenario:=house

    World files are resolved from nav2_minimal_tb3_sim, which ships the
    .sdf.xacro format required by Gazebo Sim 8 (Jazzy). The older .world
    files in turtlebot3_gazebo are Gazebo Classic only and will not work.

    To verify available worlds on your system:
        ls /opt/ros/jazzy/share/nav2_minimal_tb3_sim/worlds/
'''
...
# Scenario definitions: world path relative to the package share dir,
# plus the robot's spawn pose. Add new scenarios here as needed.
_SCENARIOS = {
    'sandbox': {
        'pkg': 'nav2_minimal_tb3_sim',
        'world_rel': 'worlds/tb3_sandbox.sdf.xacro',
        'x': '-2.0',
        'y': '-0.5',
        'yaw': '0.0',
    },
    'house': {
        'pkg': 'mission_manager',
        'world_rel': 'worlds/tb3_house.sdf.xacro',
        'x': '0.0',
        'y': '0.0',
        'yaw': '0.0',
    },
...
class SlamAndExplore(Node):
    ...
    def __init__(self):
        ...
        # Resolve and validate scenario config at startup so problems are
        # reported immediately rather than at launch time.
        self.scenario_config = self._resolve_scenario_config()
        ...
        self.get_logger().info(
            f'slam_and_explore node started. '
            f'scenario={self.scenario}  world={self.scenario_config["world"]}'
        )

    # ------------------------------------------------------------------
    # Scenario / world resolution
    # ------------------------------------------------------------------

    def _resolve_scenario_config(self) -> dict:
        '''
        Look up the scenario definition, resolve the world file path from
        the nav2_minimal_tb3_sim package share directory, and validate that
        the file actually exists on disk.

        Raises RuntimeError on any unrecoverable problem so the node fails
        fast at startup rather than silently at launch time.
        '''
        # Fall back to sandbox with a warning if an unknown scenario is given.
        if self.scenario not in _SCENARIOS:
            self.get_logger().warning(
                f"Unknown scenario '{self.scenario}'. "
                f"Valid options: {list(_SCENARIOS.keys())}. "
                f"Falling back to 'sandbox'."
            )
            self.scenario = 'sandbox'

        definition = _SCENARIOS[self.scenario]

        # Resolve the package share directory.
        try:
            pkg_share = get_package_share_directory(definition['pkg'])
        except PackageNotFoundError:
            raise RuntimeError(
                f"Package '{definition['pkg']}' not found. "
                f"Is your workspace sourced? (source /opt/ros/jazzy/setup.bash)"
            )

        world_path = os.path.join(pkg_share, definition['world_rel'])

        # Confirm the file is actually there — catches missing house world, etc.
        if not os.path.isfile(world_path):
            raise RuntimeError(
                f"World file not found for scenario '{self.scenario}': {world_path}\n"
                f"Check available worlds with: ls {pkg_share}/worlds/"
            )
    ...
    def launch_slam(self):
        ...
        gz_path = '/opt/ros/jazzy/share/turtlebot3_gazebo/models'
        existing = env.get('GZ_SIM_RESOURCE_PATH', '')
        env['GZ_SIM_RESOURCE_PATH'] = f'{existing}:{gz_path}' if existing else gz_path
        self.get_logger().info(
            f'Launching Nav2 + SLAM | scenario={self.scenario} | world={cfg["world"]}'
        )
